...
- **API-driven scope (2026-05-13):** if the backend exposes an endpoint and the behaviour is understood (DSL or blueprint backs the shape), the UI builds against it. Do not gate UI delivery on conceptual buckets like "GCL", "endorsement", or "post-issuance" when the API surface is already there. Scope locks below are now limited to capabilities the backend genuinely doesn't ship.
- **Out of V1 (backend-absent only):** real auth (Keycloak), backend-enforced maker-checker, PII/Cerbos UI gating, PDF's UW/RI review states. Items previously listed here that DO have backend APIs — GCL MemberQuote, post-issuance AddMember on live policy — moved in-scope per the API-driven-scope rule above.
...
7. **GCL endpoints:** backend endpoints exist and are DSL-canonical (`/api/quotation/member-quotes/*`, [docs/spec/quotation/MemberQuoteWorkflow.workflow](../docs/spec/quotation/MemberQuoteWorkflow.workflow)). Build UI against them per the API-driven-scope rule (2026-05-13). If real-backend behaviour is partially stubbed at runtime, surface that the same way other backend gaps are surfaced (disabled-with-tooltip), not by withholding the UI.
